        ...
        if (obj.parent == None and obj.type in ['EMPTY']):
            if(index == bpy.context.scene.tera_shape_select_index):
                ...
                if (0 <= shape.mesh_index < len(bpy.data.meshes)):
                    ...
                    if(part == 'Top'):
                        indices, coords = draw_util.get_line(
                            loc[0], loc[1], loc[2] + 1, loc[0], loc[1], loc[2] + 1.5)
                        draw_util.draw_wire_frame(
                            indices, coords, (0, 0, 1, .8))
                    if (part == 'Bottom'):
                        indices, coords = draw_util.get_line(
                            loc[0], loc[1], loc[2], loc[0], loc[1], loc[2] - .5)
                        draw_util.draw_wire_frame(
                            indices, coords, (0, 0, 1, .8))
                    if (part == 'Front'):
                        indices, coords = draw_util.get_line(
                            loc[0], loc[1] + .5, loc[2] + .5, loc[0], loc[1] + 1.5, loc[2] + .5)
                        draw_util.draw_wire_frame(
                            indices, coords, (0, 0, 1, .8))
                    if (part == 'Back'):
                        indices, coords = draw_util.get_line(
                    ...
                    if (part == 'Left'):
                        indices, coords = draw_util.get_line(
                            loc[0] - .5, loc[1], loc[2] + .5, loc[0] - 1.5, loc[1], loc[2] + .5)
                        draw_util.draw_wire_frame(
                            indices, coords, (0, 0, 1, .8))
                    if (part == 'Right'):
                        indices, coords = draw_util.get_line(
